...
local function CreatePageMenu(title, text, btnID)
...
-- Toggle the markup display in the page's text control.
local function ToggleMarkup(mode)
	local m = NBUI_NB1RightPage_Contents:GetAllowMarkupType()

	if mode == nil then
		-- Toggle existing mode.
		if m == ALLOW_MARKUP_TYPE_NONE then
			-- ALLOW_MARKUP_TYPE_ALL, ALLOW_MARKUP_TYPE_COLOR_ONLY, ALLOW_MARKUP_TYPE_NONE
			NBUI_NB1RightPage_Contents:SetAllowMarkupType(ALLOW_MARKUP_TYPE_ALL)
			NBUI.db.NB1_FormattedMode = true
		else
			NBUI_NB1RightPage_Contents:SetAllowMarkupType(ALLOW_MARKUP_TYPE_NONE)
			NBUI.db.NB1_FormattedMode = false
		end
	else
		-- Use previous mode after reload.
		if mode then
			NBUI_NB1RightPage_Contents:SetAllowMarkupType(ALLOW_MARKUP_TYPE_ALL)
		else
			NBUI_NB1RightPage_Contents:SetAllowMarkupType(ALLOW_MARKUP_TYPE_NONE)
	...
	local po = NB1_IndexPool:AcquireObject(currentlyViewing)
...
-- Create the right-click menu for the text body.
local function CreateTextMenu()
	-- Hide any other open menus.
	ClearMenu()

	AddMenuItem(GetString(SI_NBUI_MARKUP_NAME), function()
		-- Check if page text was changed.
		local page = NBUI.db.NB1Pages[currentlyViewing]
		local text = NBUI_NB1RightPage_Contents:GetText()

		if not page or text ~= page.text then
			-- Ask before reloading the page and all losing changes.
			ZO_Dialogs_ShowDialog("NBUI_NB1CONFIRM_MARKUP")
		else
			ToggleMarkup()
		end
	end)
...
NB1ConfirmMarkupDialog = {
	title = {text = GetString(SI_NBUI_MARKUP_NAME)},
	mainText = {text = 'Continue reloading the page and lose all changes?'},
	buttons = {
		[1]={
...
function Create_NB1_IndexButton(NB1_IndexPool)
	...
	button:SetHandler("OnClicked", function(self, button , ctrl , alt , shift , command)
		...
		if button == MOUSE_BUTTON_INDEX_RIGHT then
			CreatePageMenu(title, text, self.id)
			return
		end
...
---------------------------------------------------------------------------------------------------
	NBUI.NB1RightPage_Contents = WINDOW_MANAGER:CreateControlFromVirtual("NBUI_NB1RightPage_Contents", NBUI.NB1RightPage_ScrollContainer, "ZO_DefaultEditMultiLineForBackdrop")
		...
		-- Remember where mouse started, to distinguish selection from clicks.
		NBUI.NB1RightPage_Contents.CursorPosition = nil
		ZO_PreHookHandler(NBUI.NB1RightPage_Contents, 'OnMouseDown', function(self, button, ctrl, alt, shift, command)
		...
		NBUI.NB1RightPage_Contents:SetHandler("OnMouseUp", function(self, button, upInside)
			...
			if button == MOUSE_BUTTON_INDEX_LEFT and upInside then
				...
				elseif NBUI.db.NB1_ClickableLinks then
					-- Ignore selection behavior.
					if pos == self.CursorPosition then
						zo_callLater(function()
							-- ZO_LinkHandler_OnLinkClicked
							local link = NBUI.GetLink(self)
							if link then
		...
		NBUI.NB1RightPage_Contents:SetHandler("OnTextChanged", function(self)
			...
			if textLen > savedVarsStringMax then
				NBUI.NB1SavePage_Button:SetEnabled(false)
				NBUI.NB1SavePage_Button:SetAlpha(0.5)
				NBUI.NBUI_NB1RightPage_CharacterCounter:SetColor(unpack(NBUI.db.NB1_WarningColor))
			else
				NBUI.NB1SavePage_Button:SetEnabled(true)
				NBUI.NB1SavePage_Button:SetAlpha(1.0)
				NBUI.NBUI_NB1RightPage_CharacterCounter:SetColor(unpack(NBUI.db.NB1_TextColor))
			end
...
function NBUI.NB1NewPage(self, title, content)
	...
	if title then
		NBUI.NB1RightPage_Title:SetText(title)
	else
	...
	if content then
		NBUI.NB1RightPage_Contents:SetText(content)
	else
...
function NBUI.NB1SavePage(self)
	...
	if currentlyViewing == nil then	-- New page.
...
-- Try to select any page by index.
function NBUI.SelectPage(i)
	if i and i >= 1 and i <= #NBUI.db.NB1Pages then
		currentlyViewing = i
		Populate_NB1_ScrollList()
	end
end
...
-- NOTE Doesn't work with every text, and I blame the crappy UI after much testing.
...
-- Returns the link under the cursor, or nil.
-- When clicking a link, the cursor is always positioned at its start.
function NBUI.GetLink(control)
	local cursor = control:GetCursorPosition()
	local text = control:GetText()

	-- Find first link start immediately after cursor.
	local first = text:find('|H', cursor)
	if first ~= cursor + 1 then
		return nil
	end

	-- Find the link's ending after cursor.
	local last = text:find('|h', cursor)
	if last == nil then
		return nil
	end
	last = text:find('|h', last+2)
	if last == nil then
		return nil
	else
		last = last + 2
	...
	local link = text:sub(first, last)
